...
*/
  nv.utils.inheritOptions = function(target, source) {
    ...
    target._inherited = nv.utils.arrayUnique(
      ops.concat(calls).concat(inherited).concat(ops).concat(target._inherited || [])
    )
...
*/
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        switch (axis.orient()) {
          ...
          case 'bottom':
            ...
            if (rotateLabels % 360) {
              ...
              rotateLabelsRule =
                'rotate(' + rotateLabels + ' 0,' + (textHeight / 2 + axis.tickPadding()) + ')'
            ...
            if (showMaxMin) {
              ...
              axisMaxMin
                .attr('transform', function(d, i) {
                  return (
                    'translate(' +
                    nv.utils.NaNtoZero(scale(d) + (isOrdinal ? scale.rangeBand() / 2 : 0)) +
                    ',0)'
                  )
                })
                .select('text')
                ...
                .style(
                  'text-anchor',
                  rotateLabels ? (rotateLabels % 360 > 0 ? 'start' : 'end') : 'middle'
                )
                .text(function(d, i) {
                  var v = fmt(d)
                  return ('' + v).match('NaN') ? '' : v
          ...
          case 'right':
            ...
            axisLabel
              ...
              .attr(
                'y',
                rotateYLabel ? -Math.max(margin.right, width) + 12 - (axisLabelDistance || 0) : -10
          ...
          case 'left':
            ...
            axisLabel
              ...
              .attr(
                'y',
                rotateYLabel ? -Math.max(margin.left, width) + 25 - (axisLabelDistance || 0) : -10
              )
              .attr('x', rotateYLabel ? -d3.max(scale.range()) / 2 : -axis.tickPadding())
            if (showMaxMin) {
              axisMaxMin = wrap.selectAll('g.nv-axisMaxMin').data(scale.domain())
        ...
        if (showMaxMin && (axis.orient() === 'left' || axis.orient() === 'right')) {
          ...
          g
            .selectAll('g') // the g's wrapping each tick
            .each(function(d, i) {
              ...
              if (scale(d) < scale.range()[1] + 10 || scale(d) > scale.range()[0] - 10) {
                ...
                if (
                  d > 1e-10 ||
                  d < -1e-10 // accounts for minor floating point errors... though could be problematic if the scale is EXTREMELY SMALL
                )
        ...
        if (showMaxMin && (axis.orient() === 'top' || axis.orient() === 'bottom')) {
          ...
          wrap.selectAll('g.nv-axisMaxMin').each(function(d, i) {
            try {
              if (
                ...
                maxMinRange.push(scale(d) - this.getBoundingClientRect().width - 4) //assuming the max and min labels are as wide as the next tick (with an extra 4 pixels just in case) // i==0, min position
              else maxMinRange.push(scale(d) + this.getBoundingClientRect().width + 4)
            } catch (err) {
              if (
                i // i== 1, max position
              )
                maxMinRange.push(scale(d) - 4) //assuming the max and min labels are as wide as the next tick (with an extra 4 pixels just in case) // i==0, min position
          ...
          g.selectAll('g').each(function(d, i) {
            if (scale(d) < maxMinRange[0] || scale(d) > maxMinRange[1]) {
              ...
              else d3.select(this).select('text').remove() // Don't remove the ZERO line!!
            }
          })
        }
  ...
  nv.models.boxPlotChart = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        g
          .select('#nv-x-label-clip-' + boxplot.id() + ' rect')
  ...
  nv.models.candlestickBar = function() {
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        else x.range(xRange || [5 + barWidth / 2, availableWidth - barWidth / 2 - 5])
        ...
        ticks.attr('class', function(d, i, j) {
          return (
            (getOpen(d, i) > getClose(d, i) ? 'nv-tick negative' : 'nv-tick positive') +
            ' nv-tick-' +
            j +
            '-' +
            i
        ...
        var rects = tickGroups
          ...
          .attr('transform', function(d, i) {
            return (
              ...
              ',' +
              (y(getY(d, i)) -
                (getOpen(d, i) > getClose(d, i) ? y(getClose(d, i)) - y(getOpen(d, i)) : 0)) +
              ')'
            )
          })
          .attr('x', 0)
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        chart.update = function() {
          ...
          else container.transition().duration(duration).call(chart)
        }
        chart.container = this
  ...
  nv.models.discreteBar = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        var groups = wrap.select('.nv-groups').selectAll('.nv-group').data(
          function(d) {
            return d
          },
          function(d) {
  ...
  nv.models.discreteBarChart = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        g
          .select('#nv-x-label-clip-' + discretebar.id() + ' rect')
          .attr('width', x.rangeBand() * (staggerLabels ? 2 : 1))
          .attr('height', 16)
          .attr('x', -x.rangeBand() / (staggerLabels ? 1 : 2))
  ...
  nv.models.distribution = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        var availableLength =
          width - (axis === 'x' ? margin.left + margin.right : margin.top + margin.bottom),
          naxis = axis == 'x' ? 'y' : 'x',
          container = d3.select(this)
        nv.utils.initSVG(container)
        ...
        renderWatch
          .transition(distWrap.exit().selectAll('line.nv-dist' + axis), 'dist exit')
          // .transition()
          .attr(axis + '1', function(d, i) {
  ...
  nv.models.focus = function(content) {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        content.width(availableWidth).height(availableHeight).color(
          data
            .map(function(d, i) {
              return d.color || color(d, i)
            })
      ...
      selection.each(function(data) {
        ...
        node
          ...
          .on('mouseover', function(evt) {
            container
              .select('.nv-series-' + evt.seriesIndex + ' .nv-distx-' + evt.pointIndex)
              .attr('y1', evt.py)
            container
              .select('.nv-series-' + evt.seriesIndex + ' .nv-disty-' + evt.pointIndex)
              .attr('x2', evt.px)
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        if (align) {
          ...
          height =
            margin.top + margin.bottom + Math.ceil(seriesWidths.length / seriesPerRow) * versPadding
        } else {
  ...
  nv.models.historicalBar = function() {
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        y
          .domain(yDomain || d3.extent(data[0].values.map(getY).concat(forceY)))
          .range(yRange || [availableHeight, 0])
        ...
        var bars = wrap.select('.nv-bars').selectAll('.nv-bar').data(
          function(d) {
            return d
          },
          function(d, i) {
        ...
        bars
          ...
          .attr('transform', function(d, i) {
            return (
              'translate(' + (x(getX(d, i)) - availableWidth / data[0].values.length * 0.45) + ',0)'
            )
          })
          .on('mouseover', function(d, i) {
        ...
        bars
          ...
          .attr('class', function(d, i, j) {
            return (
              (getY(d, i) < 0 ? 'nv-bar negative' : 'nv-bar positive') + ' nv-bar-' + j + '-' + i
            )
          })
  ...
  nv.models.historicalBarChart = function(bar_model) {
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        bars.width(availableWidth).height(availableHeight).color(
          data
            .map(function(d, i) {
              return d.color || color(d, i)
            })
  ...
  nv.models.legend = function() {
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        else wrap.attr('transform', 'translate(' + margin.left + ',' + margin.top + ')')
  ...
  nv.models.line = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        linePaths.enter().append('path').attr('class', 'nv-line').attr(
          'd',
          d3.svg
            .line()
            .interpolate(interpolate)
  ...
  nv.models.lineChart = function(cb) {
    'use strict'
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        var availableWidth = nv.utils.availableWidth(width, container, margin),
          availableHeight =
            nv.utils.availableHeight(height, container, margin) - (focusEnable ? focus.height() : 0)
        chart.update = function() {
          if (duration === 0) {
            container.call(chart)
          } else {
        ...
        lines.width(availableWidth).height(availableHeight).color(
          data
            .map(function(d, i) {
              return d.color || color(d, i)
            })
        ...
        if (!focusEnable && focus.brush.extent() === null) {
          ...
          g
            ...
            .attr(
              'transform',
              'translate(0,' + (availableHeight + margin.bottom + focus.margin().top) + ')'
            )
            .call(focus)
        ...
        interactiveLayer.dispatch.on('elementMousemove', function(e) {
          ...
          data
            ...
            .forEach(function(series, i) {
              var extent = focus.brush.extent() !== null
                ? focus.brush.empty() ? focus.xScale().domain() : focus.brush.extent()
                : x.domain()
  ...
  nv.models.lineWithFocusChart = function(cb) {
    return nv.models
      .lineChart(function(extent) {
        cb(extent)
      })
      .margin({ bottom: 30 })
      .focusEnable(true)
  ...
  nv.models.linePlusBarChart = function() {
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        var availableWidth = nv.utils.availableWidth(width, container, margin),
          availableHeight1 =
            nv.utils.availableHeight(height, container, margin) - (focusEnable ? focusHeight : 0),
          ...
          g
            ...
            .datum(
              data.map(function(series) {
                ...
                if (switchYAxisOrder) {
                  series.key =
                    series.originalKey + (series.bar ? legendRightAxisHint : legendLeftAxisHint)
                } else {
                  series.key =
                    series.originalKey + (series.bar ? legendLeftAxisHint : legendRightAxisHint)
                }
                return series
              })
            )
        ...
        bars2.width(availableWidth).height(availableHeight2).color(
          data
            .map(function(d, i) {
              return d.color || color(d, i)
            })
        ...
        lines2.width(availableWidth).height(availableHeight2).color(
          data
            .map(function(d, i) {
              return d.color || color(d, i)
            })
        ...
        g
          ...
          .attr(
            'transform',
            'translate(0,' + (availableHeight1 + margin.bottom + margin2.top) + ')'
          )
        ...
        function onBrush() {
          ...
          bars.width(availableWidth).height(availableHeight1).color(
            data
              .map(function(d, i) {
                return d.color || color(d, i)
              })
  ...
  nv.models.multiBarHorizontal = function() {
    ...
    chart._options = Object.create(
      {},
      {
        ...
        }, // this shows the group name, seems pointless? //showBarLabels:    {get: function(){return showBarLabels;}, set: function(_){showBarLabels=_;}},
        disabled: {
          get: function() {
            return disabled
          },
  ...
  nv.models.multiBarHorizontalChart = function() {
    'use strict' //------------------------------------------------------------ //============================================================ // Public Variables with Default Settings
    ...
    controls.updateState(false) //------------------------------------------------------------ //============================================================ // Private Variables
    var stateGetter = function(data) {
      return function() {
        return {
          active: data.map(function(d) {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        if (!showLegend) {
          ...
          if (legendPosition === 'bottom') {
            ...
            g
              ...
              .attr(
                'transform',
                'translate(' + controlWidth() + ',' + (availableHeight + xAxis.height()) + ')'
              )
          } else if (legendPosition === 'top') {
            if (!marginTop && legend.height() !== margin.top) {
              margin.top = legend.height()
        ...
        g
          ...
          .attr('x2', y(0))
          .attr('y1', 0)
          .attr('y2', -availableHeight) //------------------------------------------------------------ //============================================================ // Event Handling/Dispatching (in chart's scope)
        legend.dispatch.on('stateChange', function(newState) {
          for (var key in newState) state[key] = newState[key]
          dispatch.stateChange(state)
          chart.update()
    ...
    multibar.dispatch.on('elementMousemove.tooltip', function(evt) {
      tooltip()
    }) //------------------------------------------------------------ //============================================================ // Expose Public Variables // expose chart's sub-components
    chart.dispatch = dispatch
    chart.multibar = multibar
    chart.legend = legend
    chart.controls = controls
  ...
  nv.models.multiChart = function() {
    ...
    var margin = { top: 30, right: 20, bottom: 50, left: 60 },
      ...
      useInteractiveGuideline = false,
      legendRightAxisHint = ' (right axis)',
      duration = 250 //------------------------------------------------------------ //============================================================ // Private Variables
    var x = d3.scale.linear(),
      ...
      bars1 = nv.models.multiBar().stacked(true).yScale(yScale1).duration(duration),
      bars2 = nv.models.multiBar().stacked(true).yScale(yScale2).duration(duration),
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        lines1.width(availableWidth).height(availableHeight).interpolate(interpolate).color(
          color_array.filter(function(d, i) {
            return !data[i].disabled && data[i].yAxis == 1 && data[i].type == 'line'
          })
        )
        lines2.width(availableWidth).height(availableHeight).interpolate(interpolate).color(
          color_array.filter(function(d, i) {
            return !data[i].disabled && data[i].yAxis == 2 && data[i].type == 'line'
          })
        )
        scatters1.width(availableWidth).height(availableHeight).color(
          color_array.filter(function(d, i) {
            return !data[i].disabled && data[i].yAxis == 1 && data[i].type == 'scatter'
          })
        )
        scatters2.width(availableWidth).height(availableHeight).color(
          color_array.filter(function(d, i) {
            return !data[i].disabled && data[i].yAxis == 2 && data[i].type == 'scatter'
          })
        )
        bars1.width(availableWidth).height(availableHeight).color(
          color_array.filter(function(d, i) {
            return !data[i].disabled && data[i].yAxis == 1 && data[i].type == 'bar'
          })
        )
        bars2.width(availableWidth).height(availableHeight).color(
          color_array.filter(function(d, i) {
            return !data[i].disabled && data[i].yAxis == 2 && data[i].type == 'bar'
          })
        )
        stack1.width(availableWidth).height(availableHeight).interpolate(interpolate).color(
          color_array.filter(function(d, i) {
            return !data[i].disabled && data[i].yAxis == 1 && data[i].type == 'area'
          })
        )
        stack2.width(availableWidth).height(availableHeight).interpolate(interpolate).color(
          color_array.filter(function(d, i) {
        ...
        if (bars1.stacked() && dataBars1.length) {
          ...
          if (extraValue1BarStacked.length > 0)
            extraValue1BarStacked = extraValue1BarStacked
              .reduce(function(a, b) {
        ...
        if (bars2.stacked() && dataBars2.length) {
          ...
          if (extraValue2BarStacked.length > 0)
            extraValue2BarStacked = extraValue2BarStacked
              .reduce(function(a, b) {
                return a.map(function(aVal, i) {
                  return {
                    x: aVal.x,
        ...
        yScale1
          .domain(
            yDomain1 ||
              d3.extent(d3.merge(series1).concat(extraValue1BarStacked), function(d) {
                return d.y
              })
          )
        ...
        yScale2
          .domain(
            yDomain2 ||
              d3.extent(d3.merge(series2).concat(extraValue2BarStacked), function(d) {
                return d.y
              })
          )
          .range([0, availableHeight])
        ...
        g
          .select('.nv-y1.nv-axis')
          .classed('nv-disabled', series1.length ? false : true)
          .attr('transform', 'translate(' + x.range()[0] + ',0)')
        g
          .select('.nv-y2.nv-axis')
  ...
  nv.models.ohlcBar = function() {
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        var w = availableWidth / data[0].values.length * 0.9 // Setup Scales
        x.domain(xDomain || d3.extent(data[0].values.map(getX).concat(forceX)))
        if (padData)
          x.range(
            xRange || [
        ...
        y
          ...
          .range(yRange || [availableHeight, 0]) // If scale's domain don't have a range, slightly adjust to make one... so a chart can show a single data point
        if (x.domain()[0] === x.domain()[1])
          x.domain()[0]
            ? x.domain([x.domain()[0] - x.domain()[0] * 0.01, x.domain()[1] + x.domain()[1] * 0.01])
            : x.domain([-1, 1])
        if (y.domain()[0] === y.domain()[1])
          y.domain()[0]
            ? y.domain([y.domain()[0] + y.domain()[0] * 0.01, y.domain()[1] - y.domain()[1] * 0.01])
            : y.domain([-1, 1]) // Setup containers and skeleton of chart
    ...
    nv.utils.initOptions(chart)
    return chart
  } // Code adapted from Jason Davies' "Parallel Coordinates" // http://bl.ocks.org/jasondavies/1341281
  nv.models.parallelCoordinates = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        ) //Set as true if all values on an axis are missing. // Extract the list of dimensions and create a scale for each.
        ...
        dimensionNames.forEach(function(d) {
          ...
          } //Use 90% of (availableHeight - 12) for the axis range, 12 reprensenting the space necessary to display "undefined values" text. //The remaining 10% are used to display the missingValue line.
        ...
        var axisDrag = d3.behavior
          ...
          .on('dragstart', dragStart)
          .on('drag', dragMove)
          .on('dragend', dragEnd) //Add missing value line at the bottom of the chart
        ...
        if (!isNaN(step)) {
          ...
          missingValueslineText
            .attr('y', availableHeight) //To have the text right align with the missingValues line, substract 92 representing the text size.
        ...
        function updateTicks() {
          dimensions.select('.nv-axis').each(function(d, i) {
            ...
            d3
              .select(this)
              .call(axis.scale(y[d.key]).tickFormat(d.format).tickValues(currentTicks[d.key]))
          })
        }
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        g
          .select('rect')
          .attr('width', availableWidth)
          .attr('height', availableHeight > 0 ? availableHeight : 0) // Legend
        if (!showLegend) {
          g.select('.nv-legendWrap').selectAll('*').remove()
        } else {
        ...
        parallelCoordinatesWrap.transition().call(parallelCoordinates) //============================================================ // Event Handling/Dispatching (in chart's scope) //------------------------------------------------------------ //Display reset brush button
  ...
  nv.models.pieChart = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        d3.transition(pieWrap).call(pie) //------------------------------------------------------------ //============================================================ // Event Handling/Dispatching (in chart's scope)
        legend.dispatch.on('stateChange', function(newState) {
          for (var key in newState) {
            state[key] = newState[key]
          }
  ...
  nv.models.sankey = function() {
    'use strict' // - https://github.com/soxofaan/d3-plugin-captain-sankey //------------------------------------------------------------ // Sources: // - https://bost.ocks.org/mike/sankey/ //============================================================ // Public Variables with Default Settings
    var sankey = {},
      nodeWidth = 24,
      nodePadding = 8,
      size = [1, 1],
    ...
    } //------------------------------------------------------------ //============================================================ // Private Variables // Populate the sourceLinks and targetLinks for each node. // Also, if the source and target are not objects, assume they are indices.
    ...
    } // Iteratively assign the breadth (x-position) for each node. // Nodes are assigned the maximum breadth of incoming neighbors plus one; // nodes with no incoming links are assigned breadth zero, while // nodes with no outgoing links are assigned the maximum breadth.
    function computeNodeBreadths() {
      //
      var remainingNodes = nodes, nextNodes, x = 0 // Work from left to right. // Keep updating the breath (x-position) of nodes that are target of recently updated nodes. //
    ...
    function computeNodeDepths(iterations) {
      ...
      function resolveCollisions() {
        nodesByBreadth.forEach(function(nodes) {
          var node, dy, y0 = 0, n = nodes.length, i // Push any overlapping nodes down.
    ...
    } // Compute y-offset of the source endpoint (sy) and target endpoints (ty) of links, // relative to the source/target node's y-position.
  ...
  nv.models.sankeyChart = function() {
    ...
    var margin = {
      ...
      nodePadding = 40,
      units = 'units',
      center = undefined //------------------------------------------------------------ //============================================================ // Private Variables
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        if (!dataAvailable) {
          ...
          return false
        } // No errors, continue // append the svg canvas to the page
        var svg = selection
          .append('svg')
          .attr('width', width)
          .attr('height', height)
          .append('g')
          .attr('class', 'nvd3 nv-wrap nv-sankeyChart') // Set the sankey diagram properties
        ...
        node
          ...
          .append('title')
          .text(nodeTitle) // add in the title for the nodes
        node
          .append('text')
          .attr('x', -6)
          .attr('y', function(d) {
          ...
          .attr('text-anchor', 'start') // the function for moving the nodes
        function dragmove(d) {
          d3
            .select(this)
            .attr(
  ...
  nv.models.scatter = function() {
    ...
    var margin = {
      ...
      showLabels = false //============================================================ // Private Variables //------------------------------------------------------------
    var x0,
      y0,
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        var logScale = chart.yScale().name === d3.scale.log().name ? true : false // remap and flatten the data for use in calculating the scales' domains
        ...
        z
          ...
          .range(sizeRange || _sizeRange_def) // If scale's domain don't have a range, slightly adjust to make one... so a chart can show a single data point
        ...
        function updateInteractiveLayer() {
          ...
          if (useVoronoi === true) {
            ...
            if (vertices.length < 3) {
              ...
              vertices.push([x.range()[0] - 20, y.range()[0] + 20, null, null])
              vertices.push([x.range()[1] + 20, y.range()[1] - 20, null, null])
            } // keep voronoi sections from going more than 10 outside of graph // to avoid overlap with other things like legend etc
            ...
            wrap
              .select('.nv-groups')
              .selectAll('.nv-group')
              .selectAll('.nv-point') //.data(dataWithPoints) //.style('pointer-events', 'auto') // recativate events, disabled by css
              .on('click', function(d, i) {
                //nv.log('test', d, i);
        ...
        groups
          ...
          .style('stroke-opacity', 1)
          .style('fill-opacity', 0.5) // create the points, maintaining their IDs from the original data set
        ...
        points
          .filter(function(d) {
            // getDiffs must always be called to update cache
            return (
              getDiffs(d, 'shape', getShape, 'size', getSize) || scaleDiff || sizeDiff || domainDiff
            )
          })
          .watchTransition(renderWatch, 'scatter points')
          .attr(
  ...
  nv.models.scatterChart = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        if (showDistX) {
          distX.getData(scatter.x()).scale(x).width(availableWidth).color(
            data
              .map(function(d, i) {
                return d.color || color(d, i)
              })
        ...
        if (showDistY) {
          distY.getData(scatter.y()).scale(y).width(availableHeight).color(
            data
              .map(function(d, i) {
                return d.color || color(d, i)
              })
  ...
  nv.models.sparkline = function() {
    ...
    var margin = {
      ...
      dispatch = d3.dispatch('renderEnd') //============================================================ // Private Variables //------------------------------------------------------------
    var renderWatch = nv.utils.renderWatch(dispatch)
    function chart(selection) {
      renderWatch.reset()
      selection.each(function(data) {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        g
          ...
          .attr('height', availableHeight + margin.top) //index is currently global (within the chart), may or may not keep it that way
  ...
  nv.models.stackedArea = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        gEnter.append('g').attr('class', 'nv-scatterWrap')
        wrap.attr('transform', 'translate(' + margin.left + ',' + margin.top + ')') // If the user has not specified forceY, make sure 0 is included in the domain // Otherwise, use user-specified values for forceY
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        if (!showLegend) {
          ...
          var legendWidth = showControls && legendPosition === 'top'
        ...
        g
          ...
          .attr('width', availableWidth)
          .attr('height', availableHeight)
        stacked.width(availableWidth).height(availableHeight).color(
          data
            .map(function(d, i) {
              return d.color || color(d, i)
            })
          ...
          if (showYAxis) {
            if (stacked.style() === 'expand' || stacked.style() === 'stack_percent') {
              ...
              if (!oldYTickFormat || currentFormat !== percentFormatter)
                oldYTickFormat = currentFormat //Forces the yAxis to use percentage in 'expand' mode.
        ...
        interactiveLayer.dispatch.on('elementMousemove', function(e) {
          ...
          data
            ...
            .forEach(function(series, i) {
              ...
              if (typeof singlePoint === 'undefined') singlePoint = point
              if (typeof pointXLocation === 'undefined')
                pointXLocation = chart.xScale()(chart.x()(point, pointIndex)) //If we are in 'expand' mode, use the stacked percent value instead of raw value.
  ...
  nv.models.sunburst = function() {
          ...
          if (e.depth >= d.depth) {
            ...
            arcText
              ...
              .attr('transform', function() {
                ...
                if (e.depth === 0) return 'translate(' + width / 2 * -1 + ',0)'
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        partition.value(modes[mode] || modes['count']) //reverse the drawing order so that the labels of inner //arcs are drawn on top of the outer arcs.
        ...
        cGE
          ...
          }) ///Iterating via each and selecting based on the this ///makes it work ... a cG.selectAll('path') doesn't. ///Without iteration the data (in the element) didn't update.
        cG.each(function(d) {
          d3
            .select(this)
            .select('path')
  ...
  nv.models.sunburstChart = function() {
    ...
    var margin = {
      ...
      noData = null,
      duration = 250,
      dispatch = d3.dispatch('stateChange', 'changeState', 'renderEnd') //============================================================ // Private Variables //------------------------------------------------------------
    var renderWatch = nv.utils.renderWatch(dispatch)
